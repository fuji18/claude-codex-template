...
.claude/scripts/delegate-codex.sh <mode> <target>
   ...
   ├─ 上限時: 人間がターミナルから直接叩く
   └─ 縮退時: Codex 自身が .codex/prompts の手順に従って叩く
```
...
| **C 縮退** | Claude 上限到達 | **Codex**(AGENTS.md + `.codex/prompts`) | **Codex** | Codex セルフ + CI | **Codex** | **Claude 復帰後**(§2.3) |
...
| **チケット 1 枚**(1 Issue = 1 PR) | `impl --background` | 依存なし(`depends:` 全 closed)・受け入れ条件が Issue に完結・想定差分 300 行以下・認証/決済/データ移行に触れない P1/P2 の定型機能 | Issue を読んだ第三者が質問なしで実装できると司令塔が判断したら `delegate:codex` を付ける |
...
| ~~`.codex/prompts/`~~ | — | **登録を撤回(2026-08-23、§13 #4)**: `.codex/prompts/` は Codex CLI に存在しないと実機で確定したため、マニフェストから削除した。段階5([#7](https://github.com/fuji18/claude-codex-template/issues/7))で `.codex/skills/` を実機確認したうえで改めて区分を決める |
...
- **段階3 完了(2026-08-23)**: `delegate-codex.sh impl` に終了コード `0`(完了)/`1`(判断待ち)/`2`(失敗)/`3`(利用不可)/`4`(レート上限)/`5`(計画未完成)の 6 値を実装した。`exit 0` の裏取りは事前スナップショット(作業ツリー・HEAD・tasklist の `[x]` 数)との突き合わせで行い、変化が無い委託は `failed` として弾く。`4`(レート上限)は要求定義の既知の逸脱どおり実機で再現できず、`codex` スタブでの契約確認に留めた。検収の往復回数は 0(このチケット自体の実装は判断待ちの差し戻しなく完走した)
...
5. Codex に `.codex/prompts`(or `docs/playbook/codex-standalone.md`)の手順で計画〜実装〜コミットまでさせる
